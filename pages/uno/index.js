...
const GAME_GLOW = 'rgba(229,57,53,0.35)';
...
export default function UnoLobby() {
  ...
  useEffect(() => {
    try {
      ...
      if (stored) {
        ...
        if (room.gameId === 'uno') setActiveRoom(room);
      }
    } catch {}
  }, []);
  ...
  const tabStyle = (active) => ({
    ...
    backgroundColor: 'transparent', fontFamily: 'Inter, sans-serif',
    fontWeight: active ? '700' : '500', fontSize: '15px',
    color: active ? '#ffffff' : '#8c80fc', cursor: 'pointer',
  ...
  return (
    <>
      <Head>
        <title>UNO — PoorDown</title>
        <meta name="description" content="Play UNO online with friends. Match colors and numbers, unleash wild cards!" />
      ...
      <div style={{ minHeight: '100vh', backgroundColor: '#0d0d1a', padding: '40px 20px', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: '48px', position: 'relative', zIndex: 1 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '16px', marginBottom: '12px' }}>
            <img src="/assets/uno.svg" alt="" aria-hidden="true" style={{ width: 52, filter: `drop-shadow(0 0 14px ${GAME_GLOW})` }} />
            <motion.h1
              initial={{ opacity: 0, y: -16 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.5 }}
              style={{ fontFamily: 'Nunito, sans-serif', fontSize: '64px', fontWeight: '800', color: '#ffffff', margin: 0, letterSpacing: '-2px' }}
            >
              <span style={{ color: GAME_COLOR }}>U</span>NO
            </motion.h1>
          </div>
          ...
          >
            Match colors and numbers. Play your last card to win. First to 500 points takes it all.
          </motion.p>
        </div>
        ...
        {activeRoom && (
          ...
          >
            <div>
              <p style={{ fontFamily: 'Inter, sans-serif', fontSize: '10px', fontWeight: '700', color: GAME_COLOR, letterSpacing: '1px', textTransform: 'uppercase', margin: '0 0 3px 0' }}>Active Game</p>
              <p style={{ fontFamily: 'Inter, sans-serif', fontSize: '14px', color: '#ffffff', margin: 0 }}>
                Room{' '}
                <span style={{ fontFamily: 'JetBrains Mono, monospace', fontWeight: '700', letterSpacing: '2px' }}>{activeRoom.roomCode}</span>
                {activeRoom.isHost && <span style={{ color: '#8c80fc', fontSize: '12px', marginLeft: '6px' }}>· Host</span>}
              </p>
            </div>
            <button
              onClick={() => router.push(`/uno/room/${activeRoom.roomCode}${activeRoom.isHost ? '?host=true' : ''}`)}
            ...
            {tab === 'create' ? (
              <form onSubmit={handleCreate} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                <p style={{ fontFamily: 'Inter, sans-serif', fontSize: '14px', color: '#8c80fc', margin: 0, lineHeight: 1.6 }}>
                  Create a room and share the code. Wait in the lobby until everyone joins, then the host starts the game.
                </p>
                <button
                  type="submit"
                  disabled={loading}
                  style={{ padding: '14px 24px', backgroundColor: GAME_COLOR, color: 'white', border: 'none', borderRadius: '12px', fontSize: '16px', fontWeight: '600', fontFamily: 'Inter, sans-serif', cursor: 'pointer', boxShadow: `0 4px 16px ${GAME_GLOW}`, opacity: loading ? 0.7 : 1 }}
                >
